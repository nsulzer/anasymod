...
class ModuleTop(JinjaTempl):
    ...
    def __init__(self, target):
        ...
        ports_in = []
        ports_out = []
        for port in target.cfg.ports_in:
            ports_in.append(DigitalSignal(abspath=None, width=1, name=port))
        for port in target.cfg.ports_out:
            ports_out.append(DigitalSignal(abspath=None, width=1, name=port))

        self.module_ifc = SVAPI()

        module = ModuleInst(api=self.module_ifc, name='top')
        module.add_inputs(scfg.clk_i)
        module.add_inputs(ports_in)
        module.add_outputs(ports_out)
        ...
        tb_inst.add_inputs(ports_in, connections=ports_in)
        tb_inst.add_outputs(ports_out, connections=ports_out)
